...
StaticRenderer::StaticRenderer(struct Scene *sc, struct Window *window)
{
    ...
    for (int i = 0; i < MeshComp::mPrimitivesCount; i++) {
        ...
        glBufferData(GL_ARRAY_BUFFER, MeshComp::mVertCounts[i] * sizeof(float) * 12, MeshComp::mVertices[i],
                     GL_STATIC_DRAW);

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, MeshComp::mIndCounts[i] * sizeof(int), MeshComp::mIndices[i],
                     GL_STATIC_DRAW);
